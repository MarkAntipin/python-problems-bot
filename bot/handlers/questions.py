...
async def questions_handler(update: Update, _: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if callback_questions_data:
        ...
        user_answer = callback_questions_data.answer # create new variable user_answer
        ...
        if not previous_question:
            ...
            is_correct = await questions_service.answer_question(
                ...
                user_id=user.id,
                user_answer=user_answer  # use new variable user_answer
            )

            await query.edit_message_text(
                parse_mode=ParseMode.HTML,
                text=format_explanation(question=previous_question,
                                        is_correct=is_correct, user_answer=user_answer)  # use new variable user_answer
            )
